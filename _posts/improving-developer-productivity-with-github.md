...
To protect the code that goes-in the repository, there would be gates implemented that allow code that passes a certain criteria to be pushed to the repository.
...
Lets take the case when certain artifacts are generated at build time for the application, that also needs to be source controlled. The most commo example of this could be a intermediate build artifact, required for subsequent flows.
